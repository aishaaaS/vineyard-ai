...
faster_model.load_state_dict(
    torch.load("models/FASTER_RCNN.pth", map_location=device)
)
...
def detect_faster(image):
    ...
    return (
        boxes[mask],
        round(
            float(np.mean(scores[mask])) * 100
            if len(scores[mask]) > 0 else 0,
            2
        )
    )
...
def download_report(yield_value):
    ...
    elements.append(
        Paragraph("Vineyard Yield Estimation Report", styles["Heading1"])
    )
    elements.append(Spacer(1, 12))
    elements.append(
        Paragraph(f"Estimated Yield: {yield_value} kg", styles["Normal"])
    )
